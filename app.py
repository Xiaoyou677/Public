...
def get_knowledge_base():
    ...
    # 缓存路径包含模型名，确保换模型后自动重建索引
    chroma_path = os.path.join(SCRIPT_DIR, "chroma_db_text2vec")
    bm25_path = os.path.join(SCRIPT_DIR, "bm25_index_text2vec.pkl")
    splits_path = os.path.join(SCRIPT_DIR, "splits_text2vec.pkl")
...
with tab1:
    ...
    if db is None:
        ...
        user_input = st.chat_input("请输入你的问题，例如：公司工作时间是几点？迟到10分钟怎么处理？试用期工资是转正工资的百分之多少？")
        if user_input:
                ...
                with st.chat_message("assistant"):
                    with st.spinner("正在思考..."):
                        ...
                        try:
                            ...
                            final_docs = rerank_docs(user_input, candidate_docs, embeddings, top_k=5)
...
【回答规则 - 严格遵守】
1. **必须只基于上面的参考资料回答**，绝对不能编造参考资料里没有的信息
2. **防幻觉原则**：如果参考资料里完全没有相关内容，必须明确说"根据员工手册，暂无相关规定，建议咨询HR部门"，绝对不能编造答案
3. 如果参考资料里有相关内容，就基于参考资料回答，包含具体数字、比例、时间等细节
4. 回答要分点列出，简洁明了
5. 不要添加参考资料里没有的信息，不要主观推测
6. 如果参考资料里的信息不完整，就说"根据员工手册，目前仅规定了XXX，其他细节建议咨询HR部门"
